...
`src/data/gearbox.ts` is a worked two-stage bench gearbox that exercises every
diagnostic: a handed pair of bearing caps (the swap trap), a keyed shaft (roll
matters), bolts that must follow their housing, a keep-out volume over the output
shaft, and a strict six-step build order.
