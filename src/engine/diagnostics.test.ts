...
describe('runDiagnostics — fit', () => {
  it('passes a correctly seated housing on the base plate', () => {
    ...
    // Resolve the connectors through the mate rather than by hard-coded ids:
    // connector ids are generated by the joint builder.
    const mate = gearbox.steps.flatMap((s) => s.mates).find((m) => m.id === 'm-housing')!;
    const foot = housing.connectors.find((c) => c.id === mate.a.connectorId)!;
    const seat = base.connectors.find((c) => c.id === mate.b.connectorId)!;
