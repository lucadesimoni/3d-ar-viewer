...
- **approximation — reading the camera out of a WebXR session costs ~60 ms.**
  Measured on Android, 886x1920: **47.5 ms** of readback and **13.2 ms** of
  scaling. The readback is the expensive half *only because the read must
  happen inside the XR animation frame* — that is the one moment the camera
  texture holds pixels, and `gl.readPixels` there waits for the frame's own GPU
  work. Read from the page's rAF instead it costs 10 ms and returns a black
  rectangle, which is how this was found. Sixty milliseconds is two dropped
  frames at 30 fps: fine for a capture, and the ceiling on how often presence
  checking can run. The next lever is a smaller read (a region, or
  `flushRenderer: false`, or a PBO read landing a frame later) — and it wants
  measuring, not guessing: a GPU-side resize was the obvious answer twice and
  was the wrong half both times. `src/render/babylon/cameraFrame.ts`
  A diagnostics file carries `render.frameReadbackMs`, `frameScaleMs`,
  `frameSource` (`video` / `xr-raw` / `xr-blind`) and `frameUniform`.
