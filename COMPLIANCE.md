...
| 6 | **Visualization** | Babylon renderer: `src/render/babylon/SceneManager.ts`, `meshFactory.ts`; on-part step labels `src/components/StepAnnotations.tsx` | `npm run build`, `scripts/steps-check.mjs` |
...
| 8 | **Animations** | Build fly-in + exploded view: `src/engine/animation.ts`; per-step "Show me" `src/components/useStepPreview.ts`; scrubber `src/components/ModeBar.tsx` | `src/engine/animation.test.ts`, `scripts/steps-check.mjs` |
